-- audit_logs.action is a native MySQL ENUM (see V59/V69's comment on why this column needs
-- widening on every new AuditAction literal). Adds the literals introduced for the pre-publish
-- content moderation queue and the wallet withdrawal approval flow.
