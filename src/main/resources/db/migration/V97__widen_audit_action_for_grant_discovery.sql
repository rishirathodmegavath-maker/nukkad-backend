-- The scheduled AI grant-discovery pipeline (GrantDiscoveryService) audit-logs every grant it
-- publishes or re-verifies, exactly like an admin's own actions are logged -- two new literals,
-- widening audit_logs.action exactly as every prior addition to this native MySQL ENUM has (see
-- V94's comment for why this class of column needs a migration on every new literal). No table or
...
ALTER TABLE audit_logs
    MODIFY COLUMN action ENUM(
        ...
        'AI_GRANT_DISCOVERED','AI_GRANT_VERIFIED'
    ) NOT NULL;
